...
1. Prepare the complete handoff while the issue is unassigned. Read blockers, comments, activity, `live-runs`, `active-run`, recent runs, recovery actions, executor configuration, project workspace, and review policy. Preserve the executor's existing/default environment unless the operator explicitly requests an override. Add the final brief and configure required agent reviewers through `executionPolicy.stages[].participants`. If human final acceptance is required, state that the agent reviewer must request it through `request_confirmation`; do not add a user approval stage. Read the issue back and verify there are no live runs or unresolved recovery actions.
...
4. The executor submits by transitioning to `done`. Let Paperclip intercept that transition, move the issue to `in_review`, update `executionState`, and route the configured agent reviewer. Do not manually assign or wake the reviewer unless Paperclip explicitly reports skipped or failed review dispatch.
5. When human final acceptance is required, the reviewer creates one idempotent `request_confirmation` with the review evidence, residual risks, and explicit accept/reject consequences, then waits without transitioning the issue. On acceptance, the resumed reviewer records the decision and finalizes through the native review path. On rejection, the resumed reviewer records the reason and returns the work through the native change-request path. A pending interaction is healthy waiting state; do not add the operator as reviewer/approver or finalize through CLI on the operator's behalf.
6. For a material correction, interrupt/cancel once and wait for live runs, process recovery, interaction resume attempts, comment retries, and relevant recovery actions to settle. Only then unassign, repair, verify, and reassign once.
