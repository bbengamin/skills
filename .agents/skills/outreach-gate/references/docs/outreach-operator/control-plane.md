...
- the Twenty segment pointer (how to find this run's leads in Twenty). This must be durable and read-back verifiable, such as a campaign/list/membership id or an explicit run-label field. Provider provenance alone, for example `sourcedFrom=APOLLO`, is not a segment pointer because it can mix multiple runs. When the pointer is a Twenty Campaign, it must remain `DRAFT` until push/activation is explicitly in scope; source may create memberships for segmentation, but must not bind sending accounts or send-enable the campaign.
