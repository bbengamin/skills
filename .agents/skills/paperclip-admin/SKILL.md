...
- keep ordinary execution-policy reviewers agent-only
- when human final acceptance is required, put that requirement in the handoff and require the agent reviewer to create an idempotent `request_confirmation`; do not add the operator as a user approval-stage participant
...
1. **Prepare while unassigned.** Read the issue, blockers, comments, activity, `issue live-runs`, `issue active-run`, recent `issue runs`, recovery actions, executor configuration, project workspace, and review policy. Preserve the executor's existing/default environment unless the operator explicitly requests an override. Add or update the complete handoff and configure the agent reviewer gate before assigning. If human final acceptance is required, specify the decision criteria and require the reviewer to create `request_confirmation`; do not configure a user approval stage. Read the issue back and verify the brief, status, blockers, null assignee, workspace/environment posture, and `executionPolicy`. Done when the issue is complete, verified, unassigned, and has no live or queued run or unresolved recovery action.
...
4. **Let agent review route natively.** The executor records evidence and transitions the issue to `done`. Paperclip intercepts that transition, moves the issue to `in_review`, updates `executionState`, and assigns/wakes the selected agent reviewer. Verify `executionState.currentStageType` and `currentParticipant` when diagnosing review routing. Do not manually assign, mention, resume, or wake the reviewer unless Paperclip explicitly reports that review dispatch was skipped or failed.
5. **Route human acceptance through an interaction.** When the handoff requires human final acceptance, the reviewer creates one idempotent `request_confirmation` with the evidence, residual risks, and accept/reject effects, then waits. Verify the interaction is pending and visible. Acceptance or rejection resumes the reviewer, who records the decision and uses the native approve/change-request path. Do not add the operator as reviewer/approver, manufacture a formal Approval, or finalize through CLI on the operator's behalf.
6. **Correct only from quiescence.** For an approved material correction, interrupt or cancel once, then wait until all live runs, interaction resume attempts, process recovery, comment retries, and relevant recovery actions are settled. Only then unassign, repair the issue/configuration, verify the corrected state, and reassign once. Never interrupt and immediately unassign/reassign.
